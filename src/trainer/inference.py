...
class Inference(BaseInference):
    ...
    def _process_data(self, data, validation=False):
        ...
        if self.loss_gan_t_w > 0 and validation == False:
            gan_loss = self.model(inputs, masks, guidances, targets, model_output['outputs'].detach(), model='D_t_loss')
        ...
        try:
            for i, data_loader in enumerate(test_data_loaders):
                ...
                if log is not None:
                    ...
                    log[f'{name}_mse'] = total_error / total_length
                    log[f'{name}_ssim'] = total_ssim / total_length
                    log[f'{name}_psnr'] = total_psnr / total_length
