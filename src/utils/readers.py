...
from utils.visual_helper import readCompressFlow
...
class Reader:
    def __init__(self, dir_name, read=True, max_length=None, sample_period=1, evaluate_image=False):
    ...
    def __getitem__(self, key):
        ...
        files = self.read_files(filenames)
        return files
...
class FrameReader(Reader):
    def __init__(
        ...
        scale=1, sample_period=1, evaluate_image=False
    ):
        self.resize = resize
        self.scale = scale
        self.sample_period = sample_period
        super().__init__(dir_name, read, max_length, sample_period, evaluate_image)
...
class FlowReader(Reader):
    ...
    def read_file(self, filename):
        origin_frame = readCompressFlow(filename)
        return origin_frame

    def get_samples(self, start, end):
        end = min(end, len(self.filenames)//2 + 1)
        forward_filenames = [self.filenames[i] for i in np.arange(start, end-1)*2]
        backward_filenames = [self.filenames[i] for i in (np.arange(start, end-1)*2+1)]
        return self.read_files(forward_filenames), self.read_files(backward_filenames)
...
class FlowmaskReader(FrameReader):
    def __init__(
        self, dir_name, resize=None, read=True, max_length=MAX_LENGTH,
        scale=1, sample_period=1
    ):
        super().__init__(dir_name, resize, read, max_length, scale, sample_period)
