        ...
        if args.image_masks:
            ...
            for i in range(args.n):
                ...
                masks = get_video_masks_by_moving_random_stroke(
                    ...
                    video_len=vid_lens[i], nStroke=nStroke, **preset)
...
if __name__ == "__main__":
    ...
    import glob
    davis_root = '/home/tiger/code/ffvi/data/DAVIS/Annotations/480p'
    video_folders = sorted(glob.glob(os.path.join(davis_root, '*')))
    vid_lens = [len(glob.glob(os.path.join(x, '*.jpg'))) for x in video_folders]
    main(args, vid_lens)

    # in_folder = args.output_dir
    # src_folder = '/home/tiger/code/ffvi/data/DAVIS/Annotations/480p'
    # out_folder = '/home/tiger/code/ffvi/data/DAVIS/Annotations/480p_ffvi'

    # src_folders = sorted(glob.glob(os.path.join(src_folder, '*')))
    # folders = sorted(glob.glob(os.path.join(in_folder, '*')))

    # for index, folder in enumerate(src_folders):
    #     pth = folder.replace('480p', '480p_ffvi')
    #     if not os.path.exists(pth):
    #         os.makedirs(pth)
    #     cmd = 'cp -r ' + os.path.join(folders[index], "*") + ' ' + pth
    #     os.system(cmd)