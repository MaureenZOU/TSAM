...
from model.tsm.temporal_shift import TemporalShift, FlowGatedTemporalShift, GatedTemporalShift
...
class Conv3dBlock(nn.Module):
    ...
    def forward(self, xs, flows=None):
        out = self.conv(xs)
...
class NN3Dby2D(object):
    ...
    class Base(nn.Module):
        ...
        def forward(self, xs, flows=None):
            n,c,t,h,w = xs.shape
            xs = xs.transpose(1,2).reshape(n*t,c,h,w)
            xs = self.layer(xs).reshape(n,t,-1,h,w).transpose(1,2)
    ...
    class Conv3d(Base):
        def __init__(self, in_channels, out_channels, kernel_size,
                     stride, padding, dilation, groups, bias, use_flow_tsm=False):
            super().__init__()
...
class NN3Dby2DTSM(NN3Dby2D):
    ...
    class Conv3d(NN3Dby2D.Conv3d):
        def __init__(self, in_channels, out_channels, kernel_size,
            ...
            self.use_flow_tsm = use_flow_tsm

            if use_flow_tsm == False:
                self.TemporalShift = TemporalShift
            elif use_flow_tsm == True:
                self.TemporalShift = FlowGatedTemporalShift
        ...
        def forward(self, xs, flows=None):
            ...
            if self.use_flow_tsm == False:
                xs_tsm = tsm(xs.transpose(1, 2), L, 'zero').contiguous()  
                # xs_tsm = self.TemporalShift.shift(xs, None, fold_div=4).transpose(1,2).contiguous()
            elif self.use_flow_tsm == True:
                xs_tsm = self.TemporalShift.shift(xs, None, flows, fold_div=4).transpose(1,2).contiguous()
...
class VanillaConv(nn.Module):
    def __init__(
        ...
        groups=1, bias=True, norm="SN", activation=nn.LeakyReLU(0.2, inplace=True), conv_by='3d', use_flow_tsm=False
    ):
        ...
        self.featureConv = self.module.Conv3d(
            in_channels, out_channels, kernel_size,
            stride, self.padding, dilation, groups, bias, use_flow_tsm)
...
class VanillaDeconv(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1,
        groups=1, bias=True, norm="SN", activation=nn.LeakyReLU(0.2, inplace=True),
        scale_factor=2, conv_by='3d', use_flow_tsm=False
    ):
        super().__init__()
        self.conv = VanillaConv(
            in_channels, out_channels, kernel_size, stride, padding, dilation,
            groups, bias, norm, activation, conv_by=conv_by, use_flow_tsm=use_flow_tsm)
        self.scale_factor = scale_factor

    def forward(self, xs, flows=None):
        xs_resized = F.interpolate(xs, scale_factor=(1, self.scale_factor, self.scale_factor))
        return self.conv(xs_resized, flows)
...
class GatedConv(VanillaConv):
    def __init__(
        ...
        super().__init__(
            in_channels, out_channels, kernel_size, stride, padding, dilation,
            groups, bias, norm, activation, conv_by, use_flow_tsm
        )
        if conv_by == '2dtsm':
            self.module = NN3Dby2D
    ...
    def forward(self, xs, flows=None):
        ...
        feature = self.featureConv(xs, flows)
        if self.activation:
